...
기록이 제 일을 못 하게 되면 `project register`가 성공 결과에 경고를 함께 싣는다 — 등록 자체는 정상이므로 exit code는 `0`이고, `journal_warning`과 같은 자리다.
...
| 코드 | 의미 | 조치 |
| --- | --- | --- |
| `REGISTRATION_RECORD_UNREADABLE` | 파일이 손상됐거나 읽을 수 없다 | 등록이 진행 중이 아닐 때 파일을 삭제한다. 삭제 후 첫 등록은 부재 확인 창을 다시 지불한다 |
| `REGISTRATION_RECORD_UNWRITABLE` | state directory에 쓸 수 없다 | `0700`·디스크 여유를 확인한다. 다음 등록에서 저절로 회복될 수 있다 |
| `REGISTRATION_RECORD_AT_CAPACITY` | 상한(256)에 닿았다 | 파일을 삭제해 다시 채운다. 그전까지 새 Project는 좌표를 남기지 못한다 |

경고가 없으면 기록은 정상이다. 경고를 무시해도 등록은 계속 옳지만, 중단된 등록을 재시도할 때 단건 조회로 수렴하지 못하고 창을 지불한다.
