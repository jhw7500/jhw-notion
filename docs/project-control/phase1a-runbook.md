...
등록 hint는 `$JHW_CONTROL_STATE_DIR/project-registrations.json`(`0600`)에 남는 host-local 기록이다. create **직전**에 Project ID를, create **직후**에 `{project_item_id, source_node_id}`를 durable하게 적고, 등록이 검증되면 지운다. 다음 실행은 이 기록으로 세 경우를 구분한다.

| hint 상태 | 의미 | 동작 |
| --- | --- | --- |
| 없음 (읽기·쓰기 정상) | 직전 실행이 create에 도달한 적 없음 | **부재 확인 대기 0초**로 곧바로 생성 |
| 좌표 있음 | 직전 실행이 만들었고 필드 쓰기 전에 끊김 | `node(id:)` **단건 조회**로 그 레코드를 확인해 재사용 |
| Project ID만 있음 / 좌표가 해결 안 됨 / 기록을 읽거나 쓸 수 없음 | 직전 실행 여부를 단정할 수 없음 | 기존 부재 확인 창을 그대로 지불 |

hint는 **authority가 아니라 힌트**다. 조회가 실패하거나, 다른 Project의 item을 가리키거나, 본문의 Project ID가 다르면 조용히 평소 경로로 떨어진다. 재사용 결정 자체는 hint가 아니라 기존 payload gate(제목·본문 exact match)가 내린다. 기록을 지우지 못해도 안전하다 — 다음 실행이 단건 조회 한 번을 더 하고 같은 레코드를 재사용한다.
...
잔여 위험이 남는다. 창을 타는 경우 지연이 창보다 길면 여전히 중복이 생길 수 있다. hint 기록에 **실패한** 실행이 레코드를 만들면 그 다음 실행은 hint가 비어 있다고 보고 창을 건너뛸 수 있으므로, state directory 쓰기 실패는 `0700`/디스크 상태를 즉시 확인한다. 그리고 hint도 호스트 전역 lock도 **한 호스트 안에서만** 성립하므로 다중 호스트 동시 등록은 이 보호 범위 밖이다.
