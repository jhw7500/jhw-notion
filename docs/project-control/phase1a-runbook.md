...
| 기록 상태 | 의미 | 동작 |
| --- | --- | --- |
| 파일 자체가 없음 (기능 도입 전 / 삭제 후) | 이 파일보다 오래된 레코드를 대변할 수 없음 | 그 등록이 **이미 읽은 전체 목록으로 파일을 seed**하고, 자신은 창을 지불 |
| 파일은 있고 이 Project 항목만 없음 | seed 시점에도 없었고 이 호스트가 만든 적도 없음 | **부재 확인 대기 0초**로 곧바로 생성 |
| 좌표 있음 | 이 호스트가 만들었거나 seed로 확인한 레코드가 있음 | `node(id:)` **단건 조회**로 확인해 재사용 |
| Project ID만 있음 / 좌표가 해결 안 됨 / 기록을 읽거나 쓸 수 없음 | 직전 실행 여부를 단정할 수 없음 | 기존 부재 확인 창을 그대로 지불 |

seed가 핵심이다. 파일을 만드는 등록은 그 시점 보드에 있던 **모든** Project Record의 좌표를 함께 적는다. 그래서 기능 도입 이전에 등록된 것들도 첫 등록 한 번으로 전부 기록에 들어오고, 이후 그것들의 재등록은 창이 아니라 단건 조회로 수렴한다. seed가 실패하면 파일이 만들어지지 않아 다음 등록이 다시 창을 지불한다.

기록은 **authority가 아니라 힌트**다. 조회가 실패하거나, 다른 Project의 item을 가리키거나, 본문의 Project ID가 다르면 조용히 평소 경로로 떨어진다. 재사용 결정 자체는 기록이 아니라 기존 payload gate(제목·본문 exact match)와 종료 직전의 전수 검증이 내린다. 항목은 등록에 **성공했든 시도만 했든** Project 하나당 하나씩 영구히 쌓이며, 상한(256)을 넘으면 기록을 **거부**한다 — 그 프로젝트는 좌표를 못 남겨 이후 재실행이 계속 창을 지불하지만, 레코드를 잃지는 않는다.
...
잔여 위험이 남는다. 창을 타는 경우 지연이 창보다 길면 여전히 중복이 생길 수 있다. 창을 건너뛰는 판단이 틀리는 경우는 넷이다 — 기록에 **실패한** 실행이 레코드를 만든 경우, 기록 파일을 등록 진행 중에 지운 경우, **다른 호스트**가 만든 레코드, 그리고 **seed하던 읽기에 아직 보이지 않던 레코드**다. 마지막은 파일을 만드는 그 한 번의 읽기에만 걸리는 좁은 창이다. 앞의 둘은 state directory 쓰기 실패(`0700`/디스크)를 즉시 확인해 막고, 셋째는 기록도 호스트 전역 lock도 **한 호스트 안에서만** 성립하므로 다중 호스트 동시 등록이 이 보호 범위 밖이라는 기존 한계 그대로다. 기록 실패는 현재 별도 신호 없이 등록이 다시 느려지는 것으로만 드러난다.
