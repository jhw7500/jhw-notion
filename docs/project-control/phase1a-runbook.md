...
| 코드 | 의미 | 조치 |
| --- | --- | --- |
| `REGISTRATION_RECORD_UNREADABLE` | 파일이 손상됐거나 따라갈 수 없는 경로다 | 등록이 진행 중이 아닐 때 파일을 삭제한다. 쓰기가 먼저 읽기를 하므로 **쓰기만 시도한 등록에서도 이 코드가 난다** |
| `REGISTRATION_RECORD_UNWRITABLE` | 파일 내용은 멀쩡한데 쓰지 못했다 | 디스크 여유와 **상위 경로** 권한을 확인한다. state directory 자신의 모드는 매 호출 `0700`으로 복원되므로 원인이 아니다. 다음 등록에서 저절로 회복될 수 있는 유일한 코드다 |
