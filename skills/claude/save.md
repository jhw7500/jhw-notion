---
description: "Notion 즉시 저장(record+note+delete 통합) · --db 대상DB · --delete <id> 폐기 · --hard 휴지통이동 · --force-tag 미등록 태그 자동등록"
argument-hint: "[--db <db>] [--delete <id>] [--hard] [--force-tag] [내용]"
---
...
- `/jhw:save --force-tag <내용>` — 어휘 가드 미등록 태그(tags/tool/tech_stack)를 drop 대신 Notion data source에 자동 등록 (jhw_record/jhw_note의 `allowNewTags: true`)
...
- **성과 capture**: projects 완료·주요 결과물은 코드가 git에 있어도 결과·임팩트와 함께 저장한다(git 중복 아님 — git엔 결과/임팩트가 없음). raw 커밋 로그 복제만 지양. (지식·팁=KB는 git-유추 트리비아 제외 — review.md §3.6.1)
