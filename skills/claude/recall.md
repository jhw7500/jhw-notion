...
# /jhw:recall — Notion 회상 통합

검색 / 프로젝트 컨텍스트 / 타임라인을 모드 자동 판별로 한 진입점에서 처리.

## 모드 자동 판별 규칙

1. `--mode <m>` 명시되면 그대로 사용
2. `--history` 플래그가 있으면 history 모드
3. 인자가 projects DB의 title과 정확/근사 일치하면 → **context 모드**
4. 그 외 키워드 → **search 모드**

## 사용

- `/jhw:recall raw socket` — 키워드 검색 (search)
- `/jhw:recall jhw-notion` — 프로젝트 컨텍스트 (context)
- `/jhw:recall jhw-notion --history` — 시간순 타임라인 (history)
- `/jhw:recall --mode search jhw` — 강제 검색
...
- 조회 전용. 데이터 수정 금지.
- 키워드가 projects DB title에 정확히 일치하면 context 우선 (사용자 의도 추정).
- 결과 0건이면 "검색 결과 없음" 응답.
