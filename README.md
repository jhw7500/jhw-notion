...
Claude `enforced: true`도 `enforce` mode에서만 가능하다. preflight는 standard `HOME/.claude` 설정 source(`CLAUDE_CONFIG_DIR`가 없거나 같은 절대 경로), 네 exact first-position group, 단일 owned launcher symlink와 executable core를 먼저 확인한다. global source는 absolute HOME directory chain과 current-user-owned·group/world non-writable `.claude` parent를 descriptor-relative no-follow로 열며, `settings.json`도 current-user-owned regular 단일 link·group/world non-writable인지와 읽기 전후 identity/mode/size/time 안정성을 검증한다. launcher symlink, 그 exact target, core도 각 absolute parent chain을 descriptor-bound no-follow로 열어 root/current-user ownership과 group/world non-writable mode(root-owned sticky directory 제외), 단일 link, executable regular type, bounded full-read 전후 안정성, current-path identity를 검증한다. 빌드는 새 `dist`를 `umask 022`로 생성하고 executable core를 정확히 `0755`로 고정한다. 또한 진단 cwd부터 filesystem root까지 Claude가 발견할 수 있는 `.claude/settings.json`과 `.claude/settings.local.json`을 열린 ancestor descriptor에 상대적으로 no-follow 조회한다. 현재 사용자 소유의 안정된 regular file이 아니거나 JSON·UTF-8·top-level `disableAllHooks` key가 모호하면 fail-closed하며, 어느 source든 명시 값이 정확히 `false`가 아니면 runtime probe를 시작하지 않는다. 다른 hook 내용은 실행하거나 probe 설정에 복제하지 않는다.

runtime probe는 exact owned `UserPromptSubmit` command와 동기화 blocker만 explicit `--settings`로 전달하고, settings file이 없는 private `0700` config/cwd에서 `claude -p --restricted`를 실행한다. blocker는 owned command의 완료 signal을 기다린 뒤 종료하므로 Claude의 matching-hook 병렬 실행에서도 두 응답이 모두 관찰되어야 한다. managed policy는 Claude의 restricted-mode 계약대로 계속 적용된다. 원래 `HOME`은 canonical launcher 확장에만 유지되고 child environment는 고정된 실행 PATH와 locale에 필요한 값만 allowlist하며 Claude app/session state는 임시 config로 격리해 종료 후 삭제한다. repository hook과 독립 blocker가 각각 정확히 한 번 실행되고, 실제 Claude stream의 stdout/stderr/newline shape와 모델 turn·API 시간·비용·model usage가 모두 0인 종료인지 검증한다. 이 실제 restricted invocation의 성공으로 현재 설치 binary의 필요한 capability도 함께 확인하므로 별도 version 문자열을 신뢰하지 않는다. 그 뒤 exact `PreToolUse` command 문자열을 다시 확인하고 `/usr/bin/bash`로 owned launcher와 고정 argv를 private cwd에서 직접 실행해 bounded `GUARD_PROTOCOL_MISMATCH` deny를 확인한다. 두 probe는 ambient shell, Project Control 좌표, nested Claude session, `NODE_OPTIONS`, 실 credential/provider를 상속하지 않고 non-network dummy API key, strict empty MCP config, timeout, 출력 한도를 사용한다. 추가 owned group, malformed stream, 비용/turn 발생, config override, command probe 실패는 `enforced: false`/`NO-GO`다. `execution_recheck`는 두 adapter 모두 아직 `pending`이다.
...
`jhw-control`, `jhw-control-hook`, 정확히 소유한 Claude/Codex hook group, 네 TUI의 canonical/legacy link, MCP registration은 이 저장소 소유일 때만 제거된다. 다른 파일, link, hook group, same-name MCP entry, backup은 덮어쓰거나 삭제하지 않는다. Claude/Codex 설정 root가 symlink·non-directory·외부 소유이면 private transaction을 만들기 전에 제거를 중단한다.
