...
6. Codex가 설치되어 있으면 `~/.codex/hooks.json`의 `UserPromptSubmit`·`PreToolUse`·`PostToolUse` Guard command group과 evidence-only `SessionEnd` command group을 등록한 뒤 public `jhw-control guard preflight` 실행
