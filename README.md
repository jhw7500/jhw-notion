...
Claude `enforced: true`도 `enforce` mode에서만 가능하다. preflight는 standard `HOME/.claude` 설정 source(`CLAUDE_CONFIG_DIR`가 없거나 같은 절대 경로), 네 exact first-position group, 단일 owned launcher symlink와 executable core를 먼저 확인한다. 이어 exact owned `UserPromptSubmit` group만 private `0700` 임시 config의 `0600 settings.json`과 explicit `--settings`에 복제하고, 외부 user/project/local 설정을 읽지 않는 `claude -p --restricted`를 격리된 cwd에서 inline blocking probe와 함께 실행한다. managed policy는 Claude의 restricted-mode 계약대로 계속 적용된다. 원래 `HOME`은 canonical launcher 확장에만 유지되고 child environment는 실행·locale에 필요한 값만 allowlist하며 Claude app/session state는 임시 config로 격리해 종료 후 삭제한다. repository hook과 독립 blocker가 각각 정확히 한 번 실행되고, 모델 turn·API 시간·비용·model usage가 모두 0인 종료인지 검증한다. 이 실제 restricted invocation의 성공으로 현재 설치 binary의 필요한 capability도 함께 확인하므로 별도 version 문자열을 신뢰하지 않는다. 그 뒤 저장된 exact `PreToolUse` command를 직접 실행해 bounded `GUARD_PROTOCOL_MISMATCH` deny를 확인한다. probe는 Project Control 좌표와 nested Claude session·실 credential/provider를 상속하지 않고 non-network dummy API key, strict empty MCP config, 15초 timeout, 출력 한도를 사용한다. 추가 owned group, malformed stream, 비용/turn 발생, config override, command probe 실패는 `enforced: false`/`NO-GO`다. `execution_recheck`는 두 adapter 모두 아직 `pending`이다.
