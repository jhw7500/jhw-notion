...
- recovery status의 SessionEnd reader는 secure state directory와 `guard-journal.jsonl`을 descriptor-bound/no-follow로 열고 user ownership, regular 단일-link `0600`, 8 MiB/8192행/행당 8 KiB 한계, fatal UTF-8, 전체 event schema, read 전후 identity/size/time 안정성을 모두 검증한다. exact Task/Claim 후보가 하나이고 현재 adapter/worktree/branch/HEAD/dirty/ahead/behind와 같을 때만 `recorded`이며, 그 외 결과는 `absent | ambiguous | unverified`로 축약한다. 결과는 adapter와 시각 외 Claim/Git 좌표를 재노출하지 않고 mutation action을 선택하거나 호출하지 않는다.
- Claude와 Codex의 네 hook은 각각 `~/.claude/settings.json`, `~/.codex/hooks.json`에서 adapter-specific exact group으로 소유한다. 공통 JSON transaction editor가 existing mode와 foreign lexical content/order를 보존하고 duplicate/malformed/nonregular/race에서 fail-closed한다. install 중 Claude transaction은 Codex publish와 최종 preflight가 끝날 때까지 유지해 뒤 단계 실패 시 두 config와 새 launcher를 함께 rollback하며, reinstall/uninstall은 exact owned group에만 idempotent하다.
- Claude preflight는 standard config source, exact on-disk groups, owned executable launcher/core에 더해 실제 `claude -p`의 두 `UserPromptSubmit` 응답(repository hook + inline blocker)과 0 model turn/API time/cost/model usage를 요구한다. user hook object는 private temporary `CLAUDE_CONFIG_DIR/settings.json`으로 최소 복제하고 원래 `HOME`은 canonical launcher 확장에만 유지해 Claude app/session state write를 격리·정리한다. Project/local/managed source는 대상 cwd 기준으로 계속 합성한다. 별도 direct `PreToolUse` command probe도 exact bounded deny를 반환해야 `enforced: true`다. runtime probe는 Project Control 좌표, nested Claude markers, 실제 credential/provider를 제거하고 non-network dummy key, empty MCP config, bounded output, timeout/process-group termination을 적용한다. Codex는 기존 read-only runtime inventory와 direct command probe를 유지한다.
