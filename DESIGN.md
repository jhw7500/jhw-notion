...
- Claude와 Codex의 네 hook은 각각 `~/.claude/settings.json`, `~/.codex/hooks.json`에서 adapter-specific exact group으로 소유한다. 설치된 adapter root는 current-user-owned real directory여야 한다. 공통 JSON transaction editor가 그 parent를 no-follow descriptor에 고정한 채 existing mode와 foreign lexical content/order를 보존하고 duplicate/malformed/nonregular/path-swap race에서 fail-closed한다. install 중 Claude transaction은 Codex publish와 최종 preflight가 끝날 때까지 유지해 뒤 단계 실패 시 두 config와 새 launcher를 함께 rollback하며, reinstall/uninstall은 exact owned group에만 idempotent하다.
- Claude preflight는 standard config source, exact on-disk groups, owned executable launcher/core에 더해 실제 `claude -p --restricted`의 두 `UserPromptSubmit` 응답(repository hook + inline blocker)과 0 model turn/API time/cost/model usage를 요구한다. exact owned prompt group만 private temporary config와 explicit settings에 최소 복제하고 원래 `HOME`은 canonical launcher 확장에만 유지해 Claude app/session state write를 격리·정리한다. user/project/local source는 읽지 않으며 managed policy는 restricted-mode 계약대로 적용한다. probe cwd는 임시 config이고 environment는 실행·locale 필수값만 allowlist한다. 별도 direct `PreToolUse` command probe도 exact bounded deny를 반환해야 `enforced: true`다. runtime probe는 Project Control 좌표, nested Claude markers, 실제 credential/provider를 상속하지 않고 non-network dummy key, empty MCP config, bounded output, timeout/process-group termination을 적용한다. 실제 restricted invocation 성공이 현재 binary capability 증거이므로 version 문자열을 별도 신뢰하지 않는다. Codex는 기존 read-only runtime inventory와 direct command probe를 유지한다.
