...
- 테스트: `npm test`.
- **`ControlError` 메시지는 운영자에게 도달하지 않는다.** `controlErrorResult`가 내보내는 것은 stable `code`, Claim 충돌 시의 좌표, throw 지점이 지정한 경우 등록된 `reason` 식별자(`schemas.ts`의 `ERROR_REASONS` 닫힌 어휘 — 미등록 값은 미방출), 그리고 `COMMAND_FAILED`에 한해 별도 스키마로 검증된 bounded `detail`뿐이다. `COMMAND_FAILED.detail`은 redacted `command`, nullable `exit_code`, optional 최대 512 UTF-8 bytes의 `stderr_head`만 가지며 journal의 `error_detail`에 그대로 남는다. 그러므로 운영자가 조치해야 할 것은 **코드로 구분**하고(예: `PROJECT_REGISTRATION_UNSETTLED` 대 `MISMATCH`, `REGISTRY_MOVED_DURING_READ` 대 `REGISTRY_CORRUPT`), 같은 코드 안에서 조치가 갈리면 `details.reason`에 literal 식별자를 싣는다(예: `HANDOFF_RETRY_CONFLICT`의 `git_identity_changed` 대 `legacy_dirty_evidence_ambiguous`, `WORKTREE_DIRTY`의 `handoff_copy_not_plain_file`). **reason 추가는 `ERROR_REASONS` 등록 + 스킬 문서(`skills/claude/task.md` 결과 해석) 기재가 한 세트다** — 테스트가 소스 literal → 어휘 → 문서 순의 포함을 강제한다(역방향 — 죽은 어휘 항목 — 은 검사하지 않는다). reason은 journal에도 `error_reason`으로 남는다. 메시지는 디버깅용으로만 쓴다. `errors.ts`의 리댁션이 슬래시 커맨드·비ASCII 산문을 과잉 마스킹하는 것도 같은 이유로 무해하다 — 다만 **내보내지는 좌표(브랜치명 등)는 마스킹되면 안 되고** 그 경계는 테스트가 고정한다.
